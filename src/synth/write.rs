...
impl Synth
{
   ...
   */
  pub fn write<S : IsSamples>(&mut self, samples : S) { samples.write_samples(self) }

  pub fn read_next(&mut self) -> (f32, f32) { self.handle.read_next() }

  pub fn write_cb<F : FnMut(usize, f32, f32)>(&mut self, len : usize, incr : usize, cb : F) { self.handle.write(len, incr, cb) }
  ...
  )
  {
    self
      .handle
      .write_s16(len as _, left_out, loff as _, lincr as _, right_out, roff as _, rincr as _)
  }
  ...
  )
  {
    self
      .handle
      .write_f32(len as _, left_out, loff as _, lincr as _, right_out, roff as _, rincr as _)
  }
  ...
  )
  {
    self
    .handle
    .write_f64(len as _, left_out, loff as _, lincr as _, right_out, roff as _, rincr as _)
  }
}
