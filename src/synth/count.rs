...
impl Synth
{
   ...
   */
  pub fn count_midi_channels(&self) -> usize { self.handle.count_midi_channels() }

  /**
  Returns the number of audio channels that the synthesizer uses internally
   */
  pub fn count_audio_channels(&self) -> u8 { self.handle.count_audio_channels() }

  /**
  Returns the number of audio groups that the synthesizer uses internally.
  This is usually identical to audio_channels.
   */
  pub fn count_audio_groups(&self) -> u8 { self.handle.count_audio_groups() }

  /**
  Returns the number of effects channels that the synthesizer uses internally
   */
  pub fn count_effects_channels(&self) -> u8 { self.handle.count_effects_channels() }
}
