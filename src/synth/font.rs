...
impl Synth
{
  ...
  is found.
   */
  pub fn add_font(&mut self, font : SoundFont, reset_presets : bool) -> SoundFontId { self.handle.add_font(font, reset_presets) }
   ...
   */
  pub fn count_fonts(&self) -> usize { self.handle.count_fonts() }
   ...
   */
  pub fn nth_sfont(&self, num : usize) -> Option<&SoundFont> { self.handle.nth_font(num) }

  /**
  Get a SoundFont. The SoundFont is specified by its ID.
   */
  pub fn sfont(&self, id : SoundFontId) -> Option<&SoundFont> { self.handle.sfont(id) }
  ...
  {
    self.handle.font_bank.bank_offsets.get(sfont_id).map(|o| o.offset)
  }
}
