...
{
  fn from(id : Index) -> Self { Self(id, PhantomData) }
}
...
{
  fn eq(&self, other : &TypedIndex<T>) -> bool { self.0 == other.0 }
}
...
impl<T> TypedArena<T>
{
  ...
  pub fn insert(&mut self, value : T) -> TypedIndex<T> { self.0.insert(value).into() }

  pub fn get(&self, index : TypedIndex<T>) -> Option<&T> { self.0.get(index.0) }

  pub fn remove(&mut self, index : TypedIndex<T>) -> Option<T> { self.0.remove(index.0) }
}
...
impl RangeCheck
{
  pub fn check<E, T : PartialOrd, C : std::ops::RangeBounds<T>>(range : C, value : &T, error : E) -> Result<(), E>
  {
