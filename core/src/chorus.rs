...
impl Chorus
{
  pub(crate) fn new(sample_rate : f32, active : bool) -> Self
  {
    ...
    for i in 0 .. 5
    {
      ...
      {
        let i_shifted : f64 = i as f64 - 5 as i32 as f64 / 2.0f64 + ii as f64 / ((1 as i32) << 8 as i32 - 1 as i32) as f64;
        ...
        else
        {
          chorus.sinc_table[i as usize][ii as usize] =
            (f64::sin(i_shifted * std::f64::consts::PI) as f32 as f64 / (std::f64::consts::PI * i_shifted)) as f32;
          chorus.sinc_table[i as usize][ii as usize] = (chorus.sinc_table[i as usize][ii as usize] as f64
            * (0.5f64 * (1.0f64 + f64::cos(2.0f64 * std::f64::consts::PI * i_shifted / 5 as i32 as f32 as f64))))
            as f32
        }
      }
    }
  ...
  pub(crate) fn update(&mut self)
  {
    ...
    {
      log::warn!("chorus: number blocks larger than max. allowed! Setting value to {}.", 99);
      self.new_number_blocks = 99;
    }
    if (self.new_speed_hz as f64) < 0.29f64
    {
      log::warn!("chorus: speed is too low (min {})! Setting value to min.", 0.29f64);
      self.new_speed_hz = 0.29f32
    }
    else if self.new_speed_hz > 5 as i32 as f32
    {
      log::warn!("chorus: speed must be below {} Hz! Setting value to max.", 5);
    ...
    {
      log::warn!("chorus: level must be < 10. A reasonable level is << 1! Setting it to 0.1.",);
      self.new_level = 0.1f32
    }
    self.modulation_period_samples = (self.sample_rate / self.new_speed_hz) as isize;
    modulation_depth_samples = (self.new_depth_ms as f64 / 1000.0f64 * self.sample_rate as f64) as i32;
    if modulation_depth_samples > (1 as i32) << 12 as i32 - 1 as i32
    {
      log::warn!("chorus: Too high depth. Setting it to max ({}).", (1) << 12 - 1);
    ...
    {
      self.phase[i] = (self.modulation_period_samples as f64 * i as f64 / self.number_blocks as f64) as i32 as isize;
    }
  ...
  pub(crate) fn process_mix(&mut self, in_0 : &mut [f32; 64], left_out : &mut [f32; 64], right_out : &mut [f32; 64])
  {
    for sample_index in 0 .. 64
    {
      ...
      for i in 0 .. (self.number_blocks as usize)
      {
        let mut pos_subsamples : i32 =
          ((1 as i32) << 8 as i32 - 1 as i32) * self.counter - self.lookup_tab[self.phase[i as usize] as usize];
        let mut pos_samples : i32 = pos_subsamples / ((1 as i32) << 8 as i32 - 1 as i32);
        pos_subsamples &= ((1 as i32) << 8 as i32 - 1 as i32) - 1 as i32;

        for ii in 0 .. 5
        {
          d_out += self.chorusbuf[(pos_samples & ((1 as i32) << 12 as i32 - 1 as i32) - 1 as i32) as usize]
  ...
  pub(crate) fn reset(&mut self) { self.init(); }
}
...
fn modulate_sine(buf : &mut [i32], len : usize, depth : i32)
{
  ...
  for i in 0 .. len
  {
    ...
    buf[i] = ((1.0f64 + val) * depth as f64 / 2.0f64 * ((1 as i32) << 8 as i32 - 1 as i32) as f64) as i32;
    buf[i] -= 3 as i32 * ((1 as i32) << 12 as i32 - 1 as i32) * ((1 as i32) << 8 as i32 - 1 as i32);
  }
}
...
fn modulate_triangle(buf : &mut [i32], len : i32, depth : i32)
{
  ...
  while i <= ii
  {
    val = i as f64 * 2.0f64 / len as f64 * depth as f64 * ((1 as i32) << 8 as i32 - 1 as i32) as f64;
    val2 = ((val + 0.5f64) as i32 - 3 as i32 * ((1 as i32) << 12 as i32 - 1 as i32) * ((1 as i32) << 8 as i32 - 1 as i32)) as f64;
    let fresh2 = i;
    i = i + 1;
