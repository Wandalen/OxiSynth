...
{
  fn default() -> Self { Self::new(Default::default()).unwrap() }
}
...
impl Synth
{
  ...
  pub fn send_event(&mut self, event : MidiEvent) -> Result<(), OxiError>
  {
    match event.check()?
    {
      ...
      {
        internal::pitch_bend(self.channels.get_mut(channel as usize)?, &mut self.voices, value);
      }
      ...
      {
        internal::midi::channel_pressure(self.channels.get_mut(channel as usize)?, &mut self.voices, value);
      }
      MidiEvent::PolyphonicKeyPressure { channel, key, value } =>
      {
        internal::midi::key_pressure(self.channels.get_mut(channel as usize)?, &mut self.voices, key, value);
      }
