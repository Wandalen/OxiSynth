...
impl Synth
{
  ...
  Select a sfont.
   */
  pub fn sfont_select(&mut self, channel_id : u8, sfont_id : TypedIndex<SoundFont>) -> Result<(), OxiError>
  {
    let channel = self.channels.get_mut(channel_id as usize)?;
   ...
   */
  pub fn get_program(&self, channel_id : u8) -> Result<(Option<TypedIndex<SoundFont>>, u32, u32), OxiError>
  {
    let channel = self.channels.get(channel_id as usize)?;
  ...
  {
    internal::midi::program_reset(&mut self.channels, &self.font_bank, self.settings.drums_channel_active)
  }
}
