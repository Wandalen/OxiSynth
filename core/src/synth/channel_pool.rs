...
impl ChannelPool
{
  ...
  {
    let channels = (0 .. len).map(|id| Channel::new(id, preset.clone())).collect();
    Self(channels)
  }

  pub fn get(&self, id : usize) -> Result<&Channel, OxiError> { self.0.get(id).ok_or(OxiError::ChannelOutOfRange) }
