...
use super::super::conv::{act2hz, atten2amp, cb2amp, ct2hz, ct2hz_real, pan, tc2sec, tc2sec_attack, tc2sec_delay, tc2sec_release};
...
impl Voice
{
  pub fn new(output_rate : f32, desc : VoiceDescriptor, note_id : usize) -> Voice
  {
    ...
    let synth_gain = if desc.gain < 0.0000001 { 0.0000001 } else { desc.gain };
  ...
  pub fn is_available(&self) -> bool { self.status == VoiceStatus::Clean || self.status == VoiceStatus::Off }

  pub fn is_on(&self) -> bool { self.status == VoiceStatus::On && self.volenv_section < VoiceEnvelope::Release as i32 }

  pub fn is_playing(&self) -> bool { self.status == VoiceStatus::On || self.status == VoiceStatus::Sustained }
    ...
    else
    {
      if self.volenv_section == VoiceEnvelope::Attack as i32
      {
        ...
        if self.volenv_val > 0.0
        {
          ...
          let mut env_value : f32 = -((-200.0 * f32::ln(amp) / f32::ln(10.0) - lfo) / 960.0 - 1.0) as f32;
  ...
  pub fn get_channel_id(&self) -> usize { self.channel_id }

  pub fn get_note_id(&self) -> usize { self.note_id }
  ...
  fn calculate_runtime_synthesis_parameters(&mut self, channel : &Channel)
  {
    ...
    if let Some(tuning) = tuning
    {
      self.gen[GeneratorType::Pitch as usize].val = tuning.pitch[60]
        + self.gen[GeneratorType::ScaleTune as usize].val / 100.0f32 as f64 * (tuning.pitch[self.key as usize] - tuning.pitch[60])
    }
    else
    {
      self.gen[GeneratorType::Pitch as usize].val =
        self.gen[GeneratorType::ScaleTune as usize].val * (self.key as i32 as f32 - 60.0f32) as f64 + (100.0f32 * 60.0f32) as f64
    }
  ...
  pub fn check_sample_sanity(&mut self)
  {
    ...
    if self.gen[GeneratorType::SampleMode as usize].val as i32 == LoopMode::UntilRelease as i32
      || self.gen[GeneratorType::SampleMode as usize].val as i32 == LoopMode::DuringRelease as i32
    {
      ...
      {
        self.gen[GeneratorType::SampleMode as i32 as usize].val = LoopMode::UnLooped as i32 as f64
      }

      /* The loop points may have changed. Obtain a new estimate for the loop volume. */
      /* Is the voice loop within the sample loop? */
      if self.loopstart >= self.sample.loop_start as i32 && self.loopend <= self.sample.loop_end as i32
      {
        ...
        if self.sample.amplitude_that_reaches_noise_floor_is_valid != 0
        {
          self.amplitude_that_reaches_noise_floor_loop =
            (self.sample.amplitude_that_reaches_noise_floor / self.synth_gain as f64) as f32
        }
        else
        {
          /* Worst case */
          self.amplitude_that_reaches_noise_floor_loop = self.amplitude_that_reaches_noise_floor_nonloop
        }
      }
    }
    ...
    if self.check_sample_sanity_flag & 1i32 << 1i32 != 0
    {
      ...
      {
        if self.gen[GeneratorType::SampleMode as i32 as usize].val as i32 == LoopMode::UntilRelease as i32
          || self.gen[GeneratorType::SampleMode as i32 as usize].val as i32 == LoopMode::DuringRelease as i32
        {
          self.gen[GeneratorType::SampleMode as i32 as usize].val = LoopMode::UnLooped as i32 as f64
        }
      }
  ...
  pub fn set_param(&mut self, gen : GeneratorType, nrpn_value : f32, abs : i32)
  {
    ...
    self.gen[gen as usize].flags = if abs != 0 { GEN_ABS_NRPN as i32 } else { GEN_SET as i32 } as u8;
    self.update_param(gen);
  }
  ...
  )
  {
    ...
    if self.volenv_section != VoiceEnvelope::Delay as i32
    {
      if self.volenv_section == VoiceEnvelope::Attack as i32
      {
         ...
         */
        target_amp = atten2amp(self.attenuation) * cb2amp(self.modlfo_val * -self.modlfo_to_vol) * self.volenv_val;
      ...
      else
      {
        ...
        let amp_max;
        target_amp =
          atten2amp(self.attenuation) * cb2amp(960.0f32 * (1.0f32 - self.volenv_val) + self.modlfo_val * -self.modlfo_to_vol);
        ...
        if self.has_looped
        {
          amplitude_that_reaches_noise_floor = self.amplitude_that_reaches_noise_floor_loop
        }
        else
        {
          amplitude_that_reaches_noise_floor = self.amplitude_that_reaches_noise_floor_nonloop
        }
      ...
      match current_block
      {
        ...
        _ =>
        {
          ...
          if !(self.amp == 0.0 && amp_incr == 0.0)
          {
            ...
            let fres = ct2hz(self.fres + self.modlfo_val * self.modlfo_to_fc + self.modenv_val * self.modenv_to_fc);
            ...
            if f64::abs((fres - self.last_fres) as f64) > 0.01
            {
              ...
              let omega : f32 = (2.0f64 * std::f64::consts::PI * (fres / self.output_rate) as f64) as f32;
            ...
            {
              InterpolationMethod::None => self.dsp_float_interpolate_none(&mut dsp_buf, amp_incr, phase_incr),
              InterpolationMethod::Linear => self.dsp_float_interpolate_linear(&mut dsp_buf, amp_incr, phase_incr),
              InterpolationMethod::FourthOrder => self.dsp_float_interpolate_4th_order(&mut dsp_buf, amp_incr, phase_incr),
              InterpolationMethod::SeventhOrder => self.dsp_float_interpolate_7th_order(&mut dsp_buf, amp_incr, phase_incr),
            };
  ...
  pub fn calculate_hold_decay_buffers(&mut self, gen_base : GeneratorType, gen_key2base : GeneratorType, is_decay : i32) -> i32
  {
    let mut timecents = (self.gen[gen_base as usize].val + self.gen[gen_base as usize].mod_0 + self.gen[gen_base as usize].nrpn)
      + (self.gen[gen_key2base as usize].val + self.gen[gen_key2base as usize].mod_0 + self.gen[gen_key2base as usize].nrpn)
  ...
  pub fn update_param(&mut self, gen : GeneratorType)
  {
    macro_rules! gen_sum {
      ($id: expr) => {{
        let Generator { val, mod_0, nrpn, .. } = &self.gen[$id as usize];
    ...
    match gen
    {
      ...
      GeneratorType::Attenuation =>
      {
        ...
        self.attenuation = (self.gen[GeneratorType::Attenuation as usize].val * ALT_ATTENUATION_SCALE
          + self.gen[GeneratorType::Attenuation as usize].mod_0
          + self.gen[GeneratorType::Attenuation as usize].nrpn) as f32;
      ...
      GeneratorType::Pitch | GeneratorType::CoarseTune | GeneratorType::FineTune =>
      {
        ...
        self.pitch =
          gen_sum!(GeneratorType::Pitch) + 100.0 * gen_sum!(GeneratorType::CoarseTune) + gen_sum!(GeneratorType::FineTune);
      }
      ...
      GeneratorType::OverrideRootKey =>
      {
        ...
        {
          self.root_pitch = (self.gen[GeneratorType::OverrideRootKey as usize].val * 100.0 - self.sample.pitchadj as f64) as f32
        }
        else
        {
          self.root_pitch = self.sample.origpitch as f32 * 100.0 - self.sample.pitchadj as f32
        }
      ...
      GeneratorType::StartAddrOfs | GeneratorType::StartAddrCoarseOfs =>
      {
        self.start = self
          ...
          .wrapping_add(32768 * gen_sum!(GeneratorType::StartAddrCoarseOfs) as u32) as i32;
      ...
      GeneratorType::EndAddrOfs | GeneratorType::EndAddrCoarseOfs =>
      {
        self.end = self
          ...
          .wrapping_add(32768 * gen_sum!(GeneratorType::EndAddrCoarseOfs) as u32) as i32;
        self.check_sample_sanity_flag = 1 << 0;
      }
      ...
      GeneratorType::StartLoopAddrOfs | GeneratorType::StartLoopAddrCoarseOfs =>
      {
        self.loopstart = self
          ...
          .wrapping_add(32768 * gen_sum!(GeneratorType::StartLoopAddrCoarseOfs) as u32) as i32;
        self.check_sample_sanity_flag = 1 << 0;
      }
      ...
      GeneratorType::EndLoopAddrOfs | GeneratorType::EndLoopAddrCoarseOfs =>
      {
        self.loopend = self
          ...
          .wrapping_add(32768 * gen_sum!(GeneratorType::EndLoopAddrCoarseOfs) as u32) as i32;
        self.check_sample_sanity_flag = 1 << 0;
      }
      ...
      GeneratorType::VolEnvAttack =>
      {
        ...
        let count = 1u32.wrapping_add((self.output_rate * tc2sec_attack(val) / 64.0) as u32);
        self.volenv_data[VoiceEnvelope::Attack as usize].count = count;
        self.volenv_data[VoiceEnvelope::Attack as usize].coeff = 1.0;
        self.volenv_data[VoiceEnvelope::Attack as usize].incr = if count != 0 { 1.0 / count as f32 } else { 0.0 };
      ...
      {
        let count = self.calculate_hold_decay_buffers(GeneratorType::VolEnvHold, GeneratorType::KeyToVolEnvHold, 0) as u32;
      ...
      GeneratorType::VolEnvDecay | GeneratorType::VolEnvSustain | GeneratorType::KeyToVolEnvDecay =>
      {
        ...
        let count = self.calculate_hold_decay_buffers(GeneratorType::VolEnvDecay, GeneratorType::KeyToVolEnvDecay, 1) as u32;

        self.volenv_data[VoiceEnvelope::Decay as usize].count = count;
        self.volenv_data[VoiceEnvelope::Decay as usize].coeff = 1.0;
        self.volenv_data[VoiceEnvelope::Decay as usize].incr = if count != 0 { -1.0 / count as f32 } else { 0.0 };
      ...
      GeneratorType::VolEnvRelease =>
      {
        ...
        let count = 1u32.wrapping_add((self.output_rate * tc2sec_release(val) / 64.0) as u32);
        self.volenv_data[VoiceEnvelope::Release as usize].count = count;
        self.volenv_data[VoiceEnvelope::Release as usize].coeff = 1.0;
        self.volenv_data[VoiceEnvelope::Release as usize].incr = if count != 0 { -1.0 / count as f32 } else { 0.0 };
      ...
      GeneratorType::ModEnvDelay =>
      {
        ...
        self.modenv_data[VoiceEnvelope::Delay as usize].count = (self.output_rate * tc2sec_delay(val) / 64.0) as u32;
      ...
      GeneratorType::ModEnvAttack =>
      {
        ...
        self.modenv_data[VoiceEnvelope::Attack as usize].incr = if count != 0 { 1.0 / count as f32 } else { 0.0 };
      ...
      {
        let count = self.calculate_hold_decay_buffers(GeneratorType::ModEnvHold, GeneratorType::KeyToModEnvHold, 0) as u32;
      ...
      GeneratorType::ModEnvDecay | GeneratorType::ModEnvSustain | GeneratorType::KeyToModEnvDecay =>
      {
        let count = self.calculate_hold_decay_buffers(GeneratorType::ModEnvDecay, GeneratorType::KeyToModEnvDecay, 1) as u32;
        ...
        self.modenv_data[VoiceEnvelope::Decay as usize].incr = if count != 0 { -1.0 / count as f32 } else { 0.0 };
      ...
      GeneratorType::ModEnvRelease =>
      {
        ...
        self.modenv_data[VoiceEnvelope::Release as usize].incr = if count != 0 { -1.0 / count as f32 } else { 0.0 };
