  ...
  fn new() -> Self
  {
    ...
    while i < 7
    {
      ...
      while i2 < 256 as i32
      {
        ...
        {
          v = f64::sin(i_shifted * std::f64::consts::PI) as f32 as f64 / (std::f64::consts::PI * i_shifted);
          v *= 0.5f64 * (1.0f64 + f64::cos(2.0f64 * std::f64::consts::PI * i_shifted / 7 as i32 as f32 as f64))
        }
        else
        {
          v = 1.0f64
...
#[inline(always)]
fn phase_fract(dsp_phase : usize) -> usize { dsp_phase & 0xffffffff }
...
impl Voice
{
  ...
  /// efficient.
  pub fn dsp_float_interpolate_none(&mut self, dsp_buf : &mut [f32; 64], dsp_amp_incr : f32, phase_incr : f32) -> usize
  {
    ...
    let looping = (self.gen[GEN_SAMPLEMODE as i32 as usize].val as i32 == FLUID_LOOP_DURING_RELEASE as i32
      || self.gen[GEN_SAMPLEMODE as i32 as usize].val as i32 == FLUID_LOOP_UNTIL_RELEASE as i32
        && self.volenv_section < FLUID_VOICE_ENVRELEASE as i32) as i32;

    let end_index = if looping != 0 { self.loopend - 1 as i32 } else { self.end } as usize;

    let mut dsp_i : usize = 0;
    loop
    {
      /* round to nearest point */
      let mut dsp_phase_index = (dsp_phase.wrapping_add(0x80000000 as u32 as u64) >> 32 as i32) as usize;
      ...
      while dsp_i < 64 && dsp_phase_index <= end_index
      {
        ...
        /* round to nearest point */
        dsp_phase_index = (dsp_phase.wrapping_add(0x80000000 as u32 as u64) >> 32 as i32) as usize;
      ...
      {
        dsp_phase = (dsp_phase as u64).wrapping_sub(((self.loopend - self.loopstart) as u64) << 32 as i32) as Phase as Phase;
  ...
  pub fn dsp_float_interpolate_linear(&mut self, dsp_buf : &mut [f32; 64], dsp_amp_incr : f32, phase_incr : f32) -> usize
  {
    let mut dsp_phase : Phase = self.phase;
    let dsp_data : &[i16] = &self.sample.data;
    let mut dsp_amp : f32 = self.amp;
    ...
    let mut end_index = ((if looping != 0 { (self.loopend) - 1 as i32 } else { self.end }) - 1 as i32) as usize;
    ...
    loop
    {
      ...
      while dsp_phase_index <= end_index && dsp_i < 64
      {
        ...
        dsp_buf[dsp_i] = dsp_amp * (coeffs[0] * dsp_data[dsp_phase_index] as f32 + coeffs[1] * point as f32);
        /* increment phase and amplitude */
        dsp_phase = (dsp_phase as u64).wrapping_add(dsp_phase_incr) as Phase;
        dsp_phase_index = (dsp_phase >> 32 as i32) as usize;
        /* increment amplitude */
  ...
  /// Returns number of samples processed (usually FLUID_BUFSIZE but could be
  /// smaller if end of sample occurs).
  pub fn dsp_float_interpolate_4th_order(&mut self, dsp_buf : &mut [f32; 64], dsp_amp_incr : f32, phase_incr : f32) -> usize
  {
    let mut dsp_phase : Phase = self.phase;
    let dsp_data : &[i16] = &self.sample.data;
    let mut dsp_amp : f32 = self.amp;
    ...
    let mut end_index = ((if looping != 0 { (self.loopend) - 1 as i32 } else { self.end }) - 2 as i32) as usize;
  ...
  pub fn dsp_float_interpolate_7th_order(&mut self, dsp_buf : &mut [f32; 64], dsp_amp_incr : f32, phase_incr : f32) -> usize
  {
    let dsp_data : &[i16] = &self.sample.data;
    let mut dsp_amp : f32 = self.amp;
    ...
    /* voice is currently looping? */
    let looping = (self.gen[GEN_SAMPLEMODE as usize].val as i32 == FLUID_LOOP_DURING_RELEASE as i32
      || self.gen[GEN_SAMPLEMODE as usize].val as i32 == FLUID_LOOP_UNTIL_RELEASE as i32
        && self.volenv_section < FLUID_VOICE_ENVRELEASE as i32) as i32;

    /* last index before 7th interpolation point must be specially handled */
    let mut end_index = ((if looping != 0 { self.loopend - 1 } else { self.end }) - 3) as usize;
    ...
    loop
    {
      ...
      if dsp_phase_index > end_index
      {
        dsp_phase = (dsp_phase as u64).wrapping_sub(((self.loopend - self.loopstart) as u64) << 32i32) as Phase;
