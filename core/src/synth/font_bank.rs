...
use crate::{soundfont::Preset, utils::TypedArena, SoundFont, TypedIndex};
...
impl FontBank
{
   ...
   */
  pub fn count(&self) -> usize { self.fonts.len() }

  /**
  Get a SoundFont. The SoundFont is specified by its ID.
   */
  pub fn get_font(&self, id : TypedIndex<SoundFont>) -> Option<&SoundFont> { self.fonts.get(id) }
  ...
  pub fn iter_stack(&self) -> impl Iterator<Item = &SoundFont> { self.stack.iter().filter_map(move |f| self.fonts.get(*f)) }

  pub fn preset(&self, sfont_id : TypedIndex<SoundFont>, banknum : u32, prognum : u8) -> Option<Rc<Preset>>
  {
    let sfont = self.get_font(sfont_id);
    if let Some(sfont) = sfont
    {
      let offset = self.bank_offsets.get(sfont_id).map(|o| o.offset).unwrap_or_default();
  ...
  pub fn find_preset(&self, banknum : u32, prognum : u8) -> Option<(TypedIndex<SoundFont>, Rc<Preset>)>
  {
    for id in self.stack.iter()
    {
      ...
      if let Some(sfont) = sfont
      {
        let offset = self.bank_offsets.get(*id).map(|o| o.offset).unwrap_or_default();
...
impl BankOffsets
{
   ...
   */
  pub fn get(&self, sfont_id : TypedIndex<SoundFont>) -> Option<&BankOffset> { self.0.iter().find(|x| x.sfont_id == sfont_id) }
  ...
  pub fn remove(&mut self, sfont_id : TypedIndex<SoundFont>) { self.0.retain(|x| x.sfont_id != sfont_id); }
}
