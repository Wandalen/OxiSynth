...
impl Preset
{
  pub fn import(sf2 : &soundfont::SoundFont2, preset : &soundfont::Preset, samples : &[Rc<Sample>]) -> Result<Self, ()>
  {
  ...
  pub(crate) fn global_zone(&self) -> Option<&PresetZone> { self.global_zone.as_ref() }

  pub(crate) fn zones(&self) -> &[PresetZone] { &self.zones }

  pub fn name(&self) -> &str { &self.name }

  pub fn banknum(&self) -> u32 { self.bank }
...
impl PresetZone
{
  pub fn import(name : String, sf2 : &soundfont::SoundFont2, zone : &soundfont::Zone, samples : &[Rc<Sample>])
    -> Result<Self, ()>
  {
    let mut key_low = 0;
