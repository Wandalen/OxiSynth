...
use soundfont::data::modulator::{
  ControllerPalette, GeneralPalette, Modulator as SFModulator, ModulatorSource, ModulatorTransform, SourceDirection,
...
impl Mod
{
  pub fn get_dest(&self) -> GeneratorType { self.dest }
  ...
  pub fn get_value(&self, chan : &Channel, voice : &Voice) -> f32
  {
    ...
    if self.src.controller_palette == ControllerPalette::General(GeneralPalette::NoteOnVelocity)
      ...
      && self.src.is_linear()
      && self.src2.controller_palette == ControllerPalette::General(GeneralPalette::NoteOnVelocity)
