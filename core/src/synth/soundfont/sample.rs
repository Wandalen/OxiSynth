...
impl Sample
{
  pub fn import(sample : &soundfont::data::SampleHeader, data : Rc<SampleData>) -> Result<Sample, ()>
  {
    ...
    #[cfg(feature = "sf3")]
    {
      ...
      if sample.sample_type.is_vorbis()
      {
        ...
        if sample.loop_end > sample.end || sample.loop_start >= sample.loop_end || sample.loop_start <= sample.start
        {
    ...
    {
      sample.valid = false;
      log::warn!("Ignoring sample {:?}: too few sample data points", sample.name);
      Ok(sample)
    }
    else
    {
  ...
  pub fn optimize_sample(mut self) -> Self
  {
    ...
    if self.amplitude_that_reaches_noise_floor_is_valid == 0
    {
      ...
      /* Determine the peak level */
      let peak = if peak_max > -peak_min { peak_max } else { -peak_min };
