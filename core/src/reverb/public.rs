...
impl Reverb
{
  ...
  fn set_room_size(&mut self, value : f32) { self.roomsize = value * 0.28 + 0.7; }

  /// Query the current reverb room size
  pub fn room_size(&self) -> f32 { (self.roomsize - 0.7) / 0.28 }

  /// Set the current reverb dumping
  fn set_damp(&mut self, value : f32) { self.damp = value * 1.0; }

  /// Query the current reverb dumping
  pub fn damp(&self) -> f32 { self.damp / 1.0 }
  ...
  pub fn level(&self) -> f32 { self.wet / 3.0 }

  /// Set the current reverb width
  fn set_width(&mut self, value : f32) { self.width = value; }
