...
impl<T : PartialOrd + Copy> Range<T>
{
  fn check(&self, v : T) -> Result<T, RangeError<T>>
  {
    if v < self.min
    {
      Err(RangeError::ToSmall { got : v, min : self.min })
    }
    else if v > self.max
    {
      Err(RangeError::ToBig { got : v, max : self.max })
    }
    else
    {
      Ok(v)
...
static GAIN_RANGE : Range<f32> = Range { min : 0.0, max : 10.0 };
...
impl TryFrom<SynthDescriptor> for Settings
{
  ...
  fn try_from(desc : SynthDescriptor) -> Result<Self, Self::Error>
  {
    ...
    let gain = GAIN_RANGE.check(desc.gain).map_err(|e| SettingsError::GainRange(e))?;
