...
impl Generator
{
  pub fn read(reader : &mut Reader) -> Result<Self, ParseError>
  {
    ...
    let amount = match ty
    {
      GeneratorType::KeyRange | GeneratorType::VelRange => GeneratorAmount::Range(GeneratorAmountRange {
        low : reader.read_u8()?,
        high : reader.read_u8()?,
      }),
      GeneratorType::Instrument | GeneratorType::SampleID => GeneratorAmount::U16(reader.read_u16()?),
