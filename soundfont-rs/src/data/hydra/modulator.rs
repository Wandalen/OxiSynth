...
{
  #[inline]
  pub fn is_linear(&self) -> bool { self.ty == SourceType::Linear }

  #[inline]
  pub fn is_concave(&self) -> bool { self.ty == SourceType::Concave }

  #[inline]
  pub fn is_convex(&self) -> bool { self.ty == SourceType::Convex }

  #[inline]
  pub fn is_switch(&self) -> bool { self.ty == SourceType::Switch }

  #[inline]
  pub fn is_unipolar(&self) -> bool { self.polarity == SourcePolarity::Unipolar }

  #[inline]
  pub fn is_bipolar(&self) -> bool { self.polarity == SourcePolarity::Bipolar }

  #[inline]
  pub fn is_positive(&self) -> bool { self.direction == SourceDirection::Positive }

  #[inline]
  pub fn is_negative(&self) -> bool { self.direction == SourceDirection::Negative }

  #[inline]
  pub fn is_cc(&self) -> bool
  {
    std::mem::discriminant(&self.controller_palette) == std::mem::discriminant(&ControllerPalette::Midi(0))
  }
...
impl From<u16> for ModulatorSource
{
  fn from(src : u16) -> Self
  {
    ...
    let index : u8 = (src & 0b1111111).try_into().expect("Index is longer than 7 bits!");
    ...
    let ty : u8 = (ty & 0b111111).try_into().expect("Mod source type is longer than 6 bits!");
...
impl Modulator
{
  ...
  pub fn read_all<F : Read + Seek>(pmod : &Chunk, file : &mut F) -> Result<Vec<Self>, ParseError>
  {
    ...
    else
    {
      ...
      (0 .. amount).map(|id| Self::read(&mut reader, id == amount - 1)).collect()
    }
  }
}
  ...
  pub const fn default_pitch_bend_mod(dest : GeneratorType) -> Modulator
  {
    Modulator {
      ...
      amt_src : ModulatorSource {
        index : 16,
        controller_palette : ControllerPalette::General(GeneralPalette::PitchWheelSensitivity),
        direction : Positive,
        polarity : Unipolar,
        ty : Linear,
      },
